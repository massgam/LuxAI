...
async function askText(chatId, userText, langInstruction = '') {
  ...
  const response = await openai.responses.create({
    model: TEXT_MODEL,
    input: `Sen ${BOT_NAME} adında güçlü, profesyonel bir Telegram AI asistanısın.
${langInstruction || 'Kullanıcının yazdığı dili algıla ve aynı dilde cevap ver.'}

Kurallar:
- Linkle görsel gönderme.
- Kullanıcı görsel isterse sistem otomatik görsel üretecek.
- Kullanıcı fotoğraf gönderip değişiklik isterse sistem otomatik fotoğraf düzenleyecek.
...
async function analyzeImage(base64Image, prompt, langInstruction) {
  const response = await openai.responses.create({
    model: VISION_MODEL,
    input: [
      {
        ...
        content: [{
          type: 'input_text',
          text: `${langInstruction || 'Kullanıcının dilinde cevap ver.'} Fotoğrafı dikkatli analiz et.`
        }]
      },
      {
        role: 'user',
        content: [
          { type: 'input_text', text: prompt || 'Bu fotoğrafı kısa ve net analiz et.' },
...
async function handleUserText(msg, text) {
  ...
  const langInstruction = getLangInstruction(msg);
  ...
  try {
    ...
    if (lastPhoto && isImageAnalyzeRequest(text)) {
      ...
      const answer = await analyzeImage(lastPhoto.base64, text, langInstruction);
    ...
    const reply = await askText(chatId, text, langInstruction);
