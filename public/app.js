...
async function loadAdmin() {
  ...
  try {
    ...
    loading.remove();
    if (!data.ok && data.error) return add("ai", data.error);
    renderAdmin(data);
  } catch (e) {
...
photoInput.addEventListener("change", async () => {
  ...
  reader.onload = async () => {
    ...
    try {
      ...
      const data = await res.json();
      loading.querySelector(".bubble").textContent = data.ok ? t.photoReady : (data.error || t.err);
    } catch (e) {
      loading.querySelector(".bubble").textContent = t.err;
    }
  };
